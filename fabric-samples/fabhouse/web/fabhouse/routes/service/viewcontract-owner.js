            ...
            Fabric_Client.newDefaultKeyValueStore({
                ...
                if (query_responses && query_responses.length == 1) {
                    if (query_responses[0] instanceof Error) {
                        ...
                        if(str==""){
                            console.log("query confirm contract no result");
                            res.status(404).json({error:"no such result"}); 
                        }
                        else{
                            result = JSON.parse(str);
                            res.render('viewownercontract', {result:result,username:username,userid:userid,housename:housename,housedescription:housedescription,houseaddress:houseaddress,additional:additional,ownerid:ownerid,price:price});
                        }
                    }
                } else {
